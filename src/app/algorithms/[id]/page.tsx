        ...
        <section className={styles.visualPane} aria-labelledby="visual-heading">
          ...
          {hasVisualizer(id) ? (
            ...
            <div className={styles.placeholder}>
              可視化は準備中です。現在はソート系17種・探索系(線形/二分/三分/ジャンプ/補間/指数/フィボナッチ探索/カダンのアルゴリズム/エラトステネスの篩/フェニック木/ブルームフィルタ/山登り法/焼きなまし法/勾配降下法/k近傍法)・グリッド経路探索(BFS/DFS/ダイクストラ法/A*探索)・グラフ(ベルマン・フォード法/プリム法/クラスカル法/ボルーフカ法/トポロジカルソート/カーンのアルゴリズム/Union-Find/Tarjanの強連結成分分解/Edmonds-Karp法/Dinic法/Ford-Fulkerson法/ホップクロフト・カープ法/ジョンソンのアルゴリズム/ハフマン符号化/フロイドの循環検出法/セグメント木/スキップリスト/PageRank/HITS/一貫性ハッシュ法)・動的計画法30種(...ミラー・ラビン素数判定法/ポラードのロー法/カラツバ法/RSA暗号/ルーカス・レーマー・テスト/Baby-step Giant-step法/LRUキャッシュ/TF-IDF/BM25/RRF)・木構造(二分探索木/AVL木/Treap/赤黒木/スプレー木/区間木/トライ木/Aho-Corasick法)・文字列パターンマッチング(KMP法/ラビン-カープ法/Z algorithm/ボイヤー・ムーア法)のみ対応しています。
            </div>
          )}
        </section>
