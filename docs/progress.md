...
最終更新: 2026-07-13(可視化対応アルゴリズムを81件→88件に一括拡張(7件バッチ、10〜20件ペースの3回目、データ構造カテゴリ中心)。スプレー木・区間木をTreeVisualizerに、フェニック木・ブルームフィルタをSearchVisualizerに、LRUキャッシュをDPTableVisualizerに、セグメント木・スキップリストをGraphVisualizer(ハフマン符号化と同じ「実行結果の木/グラフ構造を事前構築して固定レイアウトに使う」手法)に追加した。データ構造カテゴリ(16件)が6件→13件へと大幅に前進(残りはb-tree/kd-tree/quad-treeの3件のみ)。区間木の追加にあたり`TreeNode`型に`hi`/`maxHigh`のoptionalフィールドを追加(既存の二分探索木・AVL木・Treap・赤黒木・スプレー木は未設定のままで影響なし)。全7件をnode --experimental-strip-typesで独立実装(BST不変条件・maxHigh不変条件の直接検証、brute-force重なり区間探索、独立実装のMapベースLRU参照実装など)と突き合わせて検証済み。可視化拡張の経緯: 16件→42件→46件→49件→53件→54件→55件→56件→57件→58件→59件→60件→61件→73件→81件→88件。更新情報(RSS)画面+Vercel Edge Functions BFF、比較画面(可視化グリッド付き)、Aboutページ、Worker使い回し最適化、pixi.jsパーティクル演出も追加済み)
...
- **対応アルゴリズム: 11種類**(線形/二分/三分/ジャンプ/補間/指数/フィボナッチ探索/カダンのアルゴリズム/エラトステネスの篩/フェニック木/ブルームフィルタ)。2026-07-13にカダンのアルゴリズム・エラトステネスの篩、続いてフェニック木・ブルームフィルタを追加
...
- **対応アルゴリズム: 27種類**(0-1ナップサック問題/LCS/編集距離/硬貨両替/棒の切り出し/部分和問題/最長増加部分列(LIS)/最長回文部分列/フロイド・ワーシャル法/Sparse Table/行列連鎖乗算/卵落とし問題/最長共通部分文字列/区間スケジューリング/ユークリッドの互除法/拡張ユークリッドの互除法/繰り返し二乗法/ロシア農民の乗算法/中国剰余定理/ディフィー・ヘルマン鍵共有/ミラー・ラビン素数判定法/ポラードのロー法/カラツバ法/RSA暗号/ルーカス・レーマー・テスト/Baby-step Giant-step法/LRUキャッシュ)。2026-07-12に9種、2026-07-13に8種+6種+1種を追加
...
- **対応アルゴリズム: 17種類**(ベルマン・フォード法/プリム法/クラスカル法/トポロジカルソート/ボルーフカ法/Union-Find/Tarjanの強連結成分分解/Edmonds-Karp法/Dinic法/Ford-Fulkerson法/ホップクロフト・カープ法/カーンのアルゴリズム/ジョンソンのアルゴリズム/ハフマン符号化/フロイドの循環検出法/セグメント木/スキップリスト)。2026-07-12にトポロジカルソート・ボルーフカ法・Union-Find・Tarjanの強連結成分分解・Edmonds-Karp法・Dinic法・Ford-Fulkerson法・ホップクロフト・カープ法、2026-07-13にカーンのアルゴリズム・ジョンソンのアルゴリズム・ハフマン符号化・フロイドの循環検出法、続いてセグメント木・スキップリストを追加
...
- **可視化はソート17種+配列探索11種+グリッド経路探索4種+グラフ17種+DP27種+木構造6種+トライ木2種(トライ木/Aho-Corasick法)+文字列パターンマッチング4種の計88件**(2026-07-13、16件から大幅拡張)。残り75件は詳細ページを開いても「準備中」の破線パネルが表示されるだけ(コンテンツの充実化と可視化対応は別軸)
...
**ユーザーとの合意事項(2026-07-12)**: 3課題(a. アルゴリズム数を文字通り10倍の約1630件にする、b. 未可視化アルゴリズムの可視化実装、c. 比較画面での可視化表示)のうち**bを最優先**として着手し、可視化対応を16件→42件→46件→49件→53件→54件→55件→56件→57件→58件→59件→60件→61件→73件→81件→88件と段階的に拡張。**cも完了**(比較画面に可視化グリッドを追加)。残るはaと、bのさらなる継続。2026-07-13にユーザーから「10〜20件ずつ進めて」と明示指示があり、以降はこのペースでバッチ処理する。

1. **可視化対応アルゴリズムのさらなる拡張(現在88件/163件、bの継続)**。
