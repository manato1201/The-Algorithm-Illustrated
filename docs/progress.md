...
最終更新: 2026-07-13(可視化対応アルゴリズムを98件→103件に一括拡張(5件バッチ、10〜20件ペースの5回目、分散システムカテゴリ全件)。GraphVisualizerにブリー・アルゴリズム・二相コミット(2PC)・Raft(簡略化リーダー選出)・Paxos(単一ラウンドBasic Paxos)・ベクタークロックを追加し、分散システムカテゴリ(6件)が一貫性ハッシュ法と合わせて全件完了した。いずれもプロセス/ノードを頂点、メッセージ交換を辺として、`edgeLabels`フィールドでメッセージ種別(PREPARE/COMMIT、RequestVote/VoteGranted等)をフェーズごとに切り替える表現方法で統一。ベクタークロックのみノード上に数値を出さず(ベクトル値は`GraphFrame.distances`が単一数値しか持てないため)、説明文でベクトル全体を伝える設計にした。全5件をnode --experimental-strip-typesで独立実装(ベクタークロックの2種類の因果関係(happens-before/concurrent)を独立計算で完全一致確認、各合意アルゴリズムの過半数判定の直接検証)と突き合わせて検証済み。可視化拡張の経緯: 16件→42件→46件→49件→53件→54件→55件→56件→57件→58件→59件→60件→61件→73件→81件→88件→98件→103件。更新情報(RSS)画面+Vercel Edge Functions BFF、比較画面(可視化グリッド付き)、Aboutページ、Worker使い回し最適化、pixi.jsパーティクル演出も追加済み)
...
- **対応アルゴリズム: 30種類**(0-1ナップサック問題/LCS/編集距離/硬貨両替/棒の切り出し/部分和問題/最長増加部分列(LIS)/最長回文部分列/フロイド・ワーシャル法/Sparse Table/行列連鎖乗算/卵落とし問題/最長共通部分文字列/区間スケジューリング/ユークリッドの互除法/拡張ユークリッドの互除法/繰り返し二乗法/ロシア農民の乗算法/中国剰余定理/ディフィー・ヘルマン鍵共有/ミラー・ラビン素数判定法/ポラードのロー法/カラツバ法/RSA暗号/ルーカス・レーマー・テスト/Baby-step Giant-step法/LRUキャッシュ/TF-IDF/BM25/RRF)。2026-07-12に9種、2026-07-13に8種+6種+1種+3種を追加
...
- **対応アルゴリズム: 25種類**(ベルマン・フォード法/プリム法/クラスカル法/トポロジカルソート/ボルーフカ法/Union-Find/Tarjanの強連結成分分解/Edmonds-Karp法/Dinic法/Ford-Fulkerson法/ホップクロフト・カープ法/カーンのアルゴリズム/ジョンソンのアルゴリズム/ハフマン符号化/フロイドの循環検出法/セグメント木/スキップリスト/PageRank/HITS/一貫性ハッシュ法/ブリー・アルゴリズム/二相コミット/Raft/Paxos/ベクタークロック)。2026-07-12にトポロジカルソート・ボルーフカ法・Union-Find・Tarjanの強連結成分分解・Edmonds-Karp法・Dinic法・Ford-Fulkerson法・ホップクロフト・カープ法、2026-07-13にカーンのアルゴリズム・ジョンソンのアルゴリズム・ハフマン符号化・フロイドの循環検出法、続いてセグメント木・スキップリスト、続いてPageRank・HITS・一貫性ハッシュ法、続いてブリー・アルゴリズム・二相コミット・Raft・Paxos・ベクタークロックを追加
...
- **可視化はソート17種+配列探索15種+グリッド経路探索4種+グラフ25種+DP30種+木構造6種+トライ木2種(トライ木/Aho-Corasick法)+文字列パターンマッチング4種の計103件**(2026-07-13、16件から大幅拡張)。残り60件は詳細ページを開いても「準備中」の破線パネルが表示されるだけ(コンテンツの充実化と可視化対応は別軸)
...
**ユーザーとの合意事項(2026-07-12)**: 3課題(a. アルゴリズム数を文字通り10倍の約1630件にする、b. 未可視化アルゴリズムの可視化実装、c. 比較画面での可視化表示)のうち**bを最優先**として着手し、可視化対応を16件→42件→46件→49件→53件→54件→55件→56件→57件→58件→59件→60件→61件→73件→81件→88件→98件→103件と段階的に拡張。**cも完了**(比較画面に可視化グリッドを追加)。残るはaと、bのさらなる継続。2026-07-13にユーザーから「10〜20件ずつ進めて」と明示指示があり、以降はこのペースでバッチ処理する。

1. **可視化対応アルゴリズムのさらなる拡張(現在103件/163件、bの継続)**。
   - データ構造カテゴリは16件中13件(BST/AVL木/Treap/赤黒木/トライ木/スプレー木/区間木/フェニック木/ブルームフィルタ/LRUキャッシュ/セグメント木/スキップリスト)が対応済み、残り3件が未対応: b-tree(多分木、1頂点に複数キーを持つため既存のTreeNode(1頂点1値)では表現できず新規設計が必要)、kd-tree/quad-tree(2次元空間分割、点+領域の2D描画が必要で他のデータ構造とは異質な新規コンポーネントが要る)
   ...
   - **情報検索・ランキングカテゴリ6件中5件(TF-IDF/BM25/PageRank/HITS/RRF)が対応済み、残りはminhash-lsh(局所性鋭敏型ハッシュ)のみ**
   - 機械学習カテゴリ10件中2件(勾配降下法/k近傍法)が対応済み、残り8件が未対応: backpropagation/decision-tree/naive-bayes/perceptron等は既存インフラで対応できる見込みだが、k-means/pca/svm/random-forest等は2次元クラスタリングや多次元処理が絡み専用検討が必要
   - 最適化・確率的手法カテゴリ7件中2件(山登り法/焼きなまし法)が対応済み、残り5件: branch-and-bound/genetic-algorithm/monte-carlo/tabu-search等
   - **分散システムカテゴリ6件は2026-07-13の5回目バッチで全件(一貫性ハッシュ法/bully-algorithm/two-phase-commit/raft/paxos/vector-clocks)完了**
   - 計算幾何カテゴリ(7件)はいずれも本質的に2D座標・点/線/多角形の描画が必要で、既存4コンポーネント(SearchVisualizer/GraphVisualizer/DPTableVisualizer/TreeVisualizer)では表現できないため、新規の2D Canvas座標系コンポーネントの設計が必要(未着手、次バッチ以降の主要候補)
   - シミュレーション・群知能カテゴリ(5件: ant-colony-optimization/boids/conways-game-of-life/langtons-ant/particle-swarm-optimization)も未着手。conways-game-of-lifeとlangtons-antは2Dグリッドのセル状態遷移なので、既存のPathfindingVisualizerのグリッド描画基盤を再利用できる可能性がある
