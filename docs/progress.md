...
最終更新: 2026-07-12(ダイクストラ法の可視化を追加し、経路探索グリッドに地形コストを導入)
...
- **対応アルゴリズム: 幅優先探索(BFS)・深さ優先探索(DFS)・ダイクストラ法の3つ**
- 10行×16列の固定迷路(壁ブロックを内部に6個配置、外周は必ず開けることでstart→goalの経路を構造的に保証)上でBFS/DFS/ダイクストラ法を実行し、`{ cellStates, description }[]` のフレーム列を生成する
...
- **地形コスト(重み)を導入**(`WEIGHT_MAP`)。row9の大部分を重み5の「地形コスト高」(`difficult`状態、`stateColors.swapping`のコーラル色)にし、既定は重み1。BFS/DFSはコストを無視してその上をそのまま突っ切るが、**ダイクストラ法だけがコストの低いrow8を迂回する**——この違いが再生比較で視覚的にわかることを`node --experimental-strip-types`での直接実行で確認済み(BFS/DFSは重い地形を通過する25マスの経路、ダイクストラ法は迂回する25マス・総コスト24の経路を選択)
- BFSは歩数最短を保証、DFSは行き止まりまで掘ってから戻る、ダイクストラ法は累積コスト最小を保証——という3アルゴリズムの性質の違いが同じUIで比較できる
...
- **content/algorithms/の128件(ソート17件・探索11件以外)はMarkdown移行時の簡易生成のまま**。`## 概要`(=元のsummary)と`## 特性・トレードオフ`(TODOコメントのみ)の2見出ししかなく、`## 仕組み`セクションが存在しない。ソート・探索の2カテゴリ(計28件)は全て充実化済み
- **可視化はソート6種+経路探索3種(BFS/DFS/ダイクストラ法)+DP1種の計10件のみ**。残り153件は詳細ページを開いても「準備中」の破線パネルが表示されるだけ(コンテンツの充実化と可視化対応は別軸で、ソート・探索の記事は充実していても可視化があるのはそのうち6+3=9件)
...
1. content/algorithms/の残り128件(グラフ・動的計画法・貪欲法・文字列・データ構造・数論暗号・計算幾何・最適化確率的手法・情報検索ランキング・機械学習・デザインパターン・シミュレーション群知能・分散システムの13カテゴリ)のコンテンツ充実化。ソート・探索の2カテゴリ(計28件)が雛形になる
2. 状態スナップショットのdiffベース記録・IndexedDBキャッシュの設計・実装(Web Worker化自体は完了、Event Sourcing的な差分記録が残作業)
3. A*探索・ベルマン・フォード法など他のグラフアルゴリズム、DP(LCS等)など、可視化対応アルゴリズムをさらに拡張する。地形コストの仕組み(`WEIGHT_MAP`)はA*にも転用できる
