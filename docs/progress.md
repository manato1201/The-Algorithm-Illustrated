...
最終更新: 2026-07-12(グラフカテゴリ16件のコンテンツを完全充実化)
...
**移行内容**: 68件→163件に拡充する際に新設したカテゴリ・項目(情報検索・ランキング/機械学習/デザインパターン23種/シミュレーション・群知能/分散システムなど)はそのまま引き継いだ。全163件をMarkdownへ変換する一時スクリプトを実行し(スクリプト自体は使い捨てのため削除済み)、可視化対応済みの9件(バブル/選択/挿入/マージ/ヒープ/クイックソート・BFS・DFS・0-1ナップサック問題)は「概要・仕組み・特性トレードオフ」の3段構成で手動で書き込みを充実させた。_*続けて、ソート(残り11件)・探索(残り9件)・動的計画法(残り10件)・グラフ(ダイクストラ法以外の残り15件: ベルマン・フォード法/フロイド・ワーシャル法/プリム法/クラスカル法/トポロジカルソート/タージャンのSCC/Union-Find/フォード・ファルカーソン法/エドモンズ・カープ法/ホップクロフト・カープ法/ジョンソンのアルゴリズム/ディニッツ法/カーンのアルゴリズム/ボルーフカ法/フロイドの循環検出法)も同じ3段構成で充実化し、ソート(17件)・探索(11件)・動的計画法(11件)・グラフ(16件)の4カテゴリが全て充実済みになった_*。残り11カテゴリ・102件は `summary` から生成した簡易な本文(概要のみ+特性トレードオフはTODOコメント)のままで、今後の拡充対象。
...
- **content/algorithms/の102件(ソート17件・探索11件・動的計画法11件・グラフ16件以外)はMarkdown移行時の簡易生成のまま**。`## 概要`(=元のsummary)と`## 特性・トレードオフ`(TODOコメントのみ)の2見出ししかなく、`## 仕組み`セクションが存在しない。ソート・探索・動的計画法・グラフの4カテゴリ(計55件)は全て充実化済み
...
1. content/algorithms/の残り102件(貪欲法・文字列・データ構造・数論暗号・計算幾何・最適化確率的手法・情報検索ランキング・機械学習・デザインパターン・シミュレーション群知能・分散システムの11カテゴリ)のコンテンツ充実化。ソート・探索・動的計画法・グラフの4カテゴリ(計55件)が雛形になる
