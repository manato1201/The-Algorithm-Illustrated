...
最終更新: 2026-07-09(sample-algorithms.tsを廃止し、Markdownベースの実データモデルに移行)
...
- **content/algorithms/の154件(可視化対応9件以外)はMarkdown移行時の簡易生成のまま**。`## 概要`(=元のsummary)と`## 特性・トレードオフ`(TODOコメントのみ)の2見出ししかなく、`## 仕組み`セクションが存在しない。9件のような深さでの充実化がまだ必要
- **可視化はソート6種+経路探索2種+DP1種の計9件のみ**。残り154件は詳細ページを開いても「準備中」の破線パネルが表示されるだけ
...
1. content/algorithms/の残り154件のコンテンツ充実化(`## 仕組み`セクションの追加、TODOのままの特性・トレードオフの執筆)。可視化対応の9件が良い雛形になる
