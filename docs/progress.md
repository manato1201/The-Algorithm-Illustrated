...
最終更新: 2026-07-09(ソートカテゴリ17件のコンテンツを完全充実化)
...
- **content/algorithms/の137件(ソートカテゴリ17件・可視化対応9件以外)はMarkdown移行時の簡易生成のまま**。`## 概要`(=元のsummary)と`## 特性・トレードオフ`(TODOコメントのみ)の2見出ししかなく、`## 仕組み`セクションが存在しない。ソートカテゴリ(17件)は全て充実化済み
...
1. content/algorithms/の残り137件(探索・グラフ・動的計画法・貪欲法・文字列・データ構造・数論暗号・計算幾何・最適化確率的手法・情報検索ランキング・機械学習・デザインパターン・シミュレーション群知能・分散システムの14カテゴリ)のコンテンツ充実化。ソートカテゴリ17件が雛形になる
