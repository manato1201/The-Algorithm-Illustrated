...
最終更新: 2026-07-13(可視化対応アルゴリズムを88件→98件に一括拡張(10件バッチ、10〜20件ペースの4回目)。情報検索・ランキング/機械学習/最適化・確率的手法/分散システムの4カテゴリに初めて可視化対応を追加した。DPTableVisualizerにTF-IDF・BM25・RRF、GraphVisualizerにPageRank・HITS・一貫性ハッシュ法、SearchVisualizerに山登り法・焼きなまし法・勾配降下法・k近傍法を追加。山登り法と焼きなまし法は同じ地形・同じ開始位置を使い、山登り法が局所最適(高さ12)で止まる一方、焼きなまし法(決定論的LCGで再現可能な乱数、seed=92)は確率的な悪化受理により大域最適(高さ15)へ脱出できることを実際に対比できる。全10件をnode --experimental-strip-typesで独立実装(独立実装のPageRank/HITS参照実装、brute-force距離計算によるk近傍法検証、局所最適の不変条件チェックなど)と突き合わせて検証済み。可視化拡張の経緯: 16件→42件→46件→49件→53件→54件→55件→56件→57件→58件→59件→60件→61件→73件→81件→88件→98件。更新情報(RSS)画面+Vercel Edge Functions BFF、比較画面(可視化グリッド付き)、Aboutページ、Worker使い回し最適化、pixi.jsパーティクル演出も追加済み)
...
- **対応アルゴリズム: 20種類**(ベルマン・フォード法/プリム法/クラスカル法/トポロジカルソート/ボルーフカ法/Union-Find/Tarjanの強連結成分分解/Edmonds-Karp法/Dinic法/Ford-Fulkerson法/ホップクロフト・カープ法/カーンのアルゴリズム/ジョンソンのアルゴリズム/ハフマン符号化/フロイドの循環検出法/セグメント木/スキップリスト/PageRank/HITS/一貫性ハッシュ法)。2026-07-12にトポロジカルソート・ボルーフカ法・Union-Find・Tarjanの強連結成分分解・Edmonds-Karp法・Dinic法・Ford-Fulkerson法・ホップクロフト・カープ法、2026-07-13にカーンのアルゴリズム・ジョンソンのアルゴリズム・ハフマン符号化・フロイドの循環検出法、続いてセグメント木・スキップリスト、続いてPageRank・HITS・一貫性ハッシュ法を追加
...
- **可視化はソート17種+配列探索15種+グリッド経路探索4種+グラフ20種+DP30種+木構造6種+トライ木2種(トライ木/Aho-Corasick法)+文字列パターンマッチング4種の計98件**(2026-07-13、16件から大幅拡張)。残り65件は詳細ページを開いても「準備中」の破線パネルが表示されるだけ(コンテンツの充実化と可視化対応は別軸)
...
**ユーザーとの合意事項(2026-07-12)**: 3課題(a. アルゴリズム数を文字通り10倍の約1630件にする、b. 未可視化アルゴリズムの可視化実装、c. 比較画面での可視化表示)のうち**bを最優先**として着手し、可視化対応を16件→42件→46件→49件→53件→54件→55件→56件→57件→58件→59件→60件→61件→73件→81件→88件→98件と段階的に拡張。**cも完了**(比較画面に可視化グリッドを追加)。残るはaと、bのさらなる継続。2026-07-13にユーザーから「10〜20件ずつ進めて」と明示指示があり、以降はこのペースでバッチ処理する。

1. **可視化対応アルゴリズムのさらなる拡張(現在98件/163件、bの継続)**。
   ...
   - 分散システムカテゴリ6件中1件(一貫性ハッシュ法)が対応済み、残り5件: bully-algorithm/paxos/raft/two-phase-commit/vector-clocksはメッセージシーケンス図的な可視化が必要で、既存のGraphVisualizerで頂点=ノード・辺=メッセージとして表現できる見込み(次のバッチ候補)
   - 計算幾何カテゴリ(7件)はいずれも本質的に2D座標・点/線/多角形の描画が必要で、既存3コンポーネント(SearchVisualizer/GraphVisualizer/DPTableVisualizer/TreeVisualizer)では表現できないため、新規の2D Canvas座標系コンポーネントの設計が必要(未着手)
