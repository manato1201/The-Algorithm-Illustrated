...
- **可視化はソート23種(新設gnome-sort/odd-even-sort/patience-sorting/tournament-sort/pigeonhole-sort/flashsort含む)+配列探索21種(新設galloping-search含む)+グリッド経路探索9種(新設best-first-search/bidirectional-search含む)+グラフ34種(新設minimax/alpha-beta-pruning/monte-carlo-tree-search/thompson-construction/subset-construction/dfa-minimization含む)+DP39種+木構造6種+トライ木2種+文字列パターンマッチング5種の計139件**(2026-07-13)。残り178件(デザインパターン23件を除く実質可視化候補は約178件)は詳細ページを開いても「準備中」の破線パネルが表示されるだけ(コンテンツの充実化と可視化対応は別軸)。ゲーム木データセット(GAME_TREE_NODES/EDGES、末端値`[2,3,8,1,7,4,6,9]`)はminimax/alpha-beta-pruning/monte-carlo-tree-searchの3アルゴリズムで共有、NFA/DFAデータセット(正規表現`a(b|c)*`)はthompson-construction→subset-construction→dfa-minimizationの3アルゴリズムでパイプライン的に共有、求根関数`f(x)=x³-x-2`はnewton-method/bisection-method/secant-methodの3アルゴリズムで共有——「同じ入力を複数アルゴリズムで比較できるようにする」という可視化の設計方針を、新設9カテゴリでも踏襲した
...
3. **可視化(課題b)を継続**。現在139件/317件(デザインパターン23件を除く実質可視化候補は約178件)。**ゲーム(minimax/alpha-beta-pruning/monte-carlo-tree-search)・コンパイラ構文解析の一部(thompson-construction/subset-construction/dfa-minimization)・数値計算の一部(newton-method/bisection-method/secant-method)の9件、深堀りバッチ1のソート6種+パス探索2種+配列探索1種の9件は2026-07-13完了**。次の有力候補: 深堀りバッチ1の残り45件のうち、GraphVisualizerで表現しやすいグラフ9種(spfa=ベルマン・フォード法と同系統/hungarian-algorithm/stable-marriage-problem/push-relabel-max-flow=既存Dinic・Edmonds-Karpと同系統/articulation-points・bridges-finding=Tarjanと同系統のDFS低リンク法/hierholzer-algorithm/two-sat/reverse-delete-algorithm=Kruskalの逆)、DPTableVisualizerで表現しやすいDP7種(word-break-problem/palindrome-partitioning/unbounded-knapsack/partition-problem/assembly-line-scheduling/optimal-binary-search-tree/burst-balloons-dp)・貪欲法2種(fractional-knapsack/job-sequencing-with-deadlines)、新設9カテゴリの残り(バイオインフォマティクスのneedleman-wunsch/smith-waterman等)、データ構造・文字列の残りの数件、機械学習の多次元手法4件、計算幾何7件+シミュレーション群知能2件(新規2Dコンポーネントが必要)
