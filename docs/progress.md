...
- **可視化はソート17種+配列探索17種+グリッド経路探索7種+グラフ28種+DP39種+木構造6種+トライ木2種(トライ木/Aho-Corasick法)+文字列パターンマッチング5種の計121件**(2026-07-13、16件から大幅拡張)。残り42件は詳細ページを開いても「準備中」の破線パネルが表示されるだけ(コンテンツの充実化と可視化対応は別軸)
