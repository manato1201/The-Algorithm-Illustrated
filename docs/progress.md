...
- **content/algorithms/は317件(24カテゴリ)**。既存163件(15カテゴリ)は`## 概要`・`## 仕組み`・`## 特性・トレードオフ`の3見出し構成でコンテンツ充実化済み(2026-07-12完了)。新設した「ゲーム」(12件)・「数値計算」(12件)・「コンピュータビジョン」(12件)・「自然言語処理」(12件)・「コンパイラ・構文解析」(12件)・「バイオインフォマティクス」(10件)・「並行処理・並列アルゴリズム」(10件)・「スケジューリング」(10件)・「制御・ロボティクス」(10件)の9カテゴリ(計100件、2026-07-13)も同じ3見出し構成で新規作成済み。**「深堀りバッチ1」(2026-07-13、263件→317件)で既存9カテゴリ(ソート/探索/グラフ/データ構造/数論・暗号/文字列/動的計画法/貪欲法/最適化・確率的手法)に計54件を追加**、教科書的な定番アルゴリズムのうち未収録だったものを中心にカテゴリの深さを拡充した。デザインパターンの`## 特性・トレードオフ`は他カテゴリのBig-O計算量ではなく、パターン固有のトレードオフ(拡張性とクラス数増加の綱引き、カプセル化との緊張関係など)を軸に記述している
...
2. **アルゴリズム数を10倍(約1630件)に拡大する**(課題a、着手済み・317件/1630件)。カテゴリ・サブカテゴリ基盤(`CATEGORY_TAXONOMY`)は確立済み、**新カテゴリ追加フェーズは2026-07-13時点で一区切り**(全24カテゴリ、ロードマップに挙げていた候補は出し尽くした)。以降は以下を並行して進める:
   - **既存24カテゴリそれぞれのサブカテゴリ内でのバリエーション追加(深堀り)が次の主軸。深堀りバッチ1(2026-07-13、263件→317件)で9カテゴリ(ソート/探索/グラフ/データ構造/数論・暗号/文字列/動的計画法/貪欲法/最適化・確率的手法)に計54件を追加済み**。残る15カテゴリ(貪欲法・最適化・数論・暗号・文字列・動的計画法以外の全て: 計算幾何/情報検索・ランキング/機械学習/デザインパターン/シミュレーション・群知能/分散システム/ゲーム/数値計算/コンピュータビジョン/自然言語処理/コンパイラ・構文解析/バイオインフォマティクス/並行処理・並列アルゴリズム/スケジューリング/制御・ロボティクス)はまだ深堀り未着手——例えばグラフの「最短路」ならSPFA(深堀りバッチ1で対応済み)に続き多始点ダイクストラ等、ゲームの「ゲームAI・意思決定」ならUCT・反復深化ミニマックス等、コンパイラ・構文解析の「字句解析」ならAho-Corasick+DFA融合等、バイオインフォマティクスの「構造予測」ならZukerアルゴリズム(Nussinov法の自由エネルギー拡張版)等、教科書に載っている定番アルゴリズムのうちまだ収録していないものをカテゴリごとに掘り下げる。深堀りに完全に上限はないため、目安として各カテゴリ+5〜10件程度を1バッチの単位とする
