...
- `id in SORT_VISUALIZERS` → `SortVisualizer`、`id in PATHFINDING_VISUALIZERS` → `PathfindingVisualizer`、どちらでもなければ破線パネルで「準備中」を表示、の順で判定する簡易ディスパッチ
...
- `SortVisualizer`・`PathfindingVisualizer`はどちらもこの2つを共用し、キャンバス描画ロジックだけを個別に持つ構成
...
- **対応アルゴリズム: バブルソート・選択ソート・挿入ソート・クイックソートの4つ**(マージソート・ヒープソートは未対応、次候補)
...
- **対応アルゴリズム: 幅優先探索(BFS)・深さ優先探索(DFS)の2つ**
- 10行×16列の固定迷路(壁ブロックを内部に6個配置、外周は必ず開けることでstart→goalの経路を構造的に保証)上でBFS/DFSを実行し、`{ cellStates, description }[]` のフレーム列を生成する
- セルの色分けはソート可視化と同じ`stateColors`を再利用: 次の候補=pivot(電光青)、探索済み=comparing(amber-yellow)、最短経路=settled(green)。スタート/ゴールのみ`coreColors.accentAmber`/`accentGreen`の専用色
- BFSは最短経路を保証、DFSは行き止まりまで掘ってから戻る挙動の違いが再生比較で分かるようにしている
- 迷路は固定のため、シャッフルに相当する操作はなく「最初から」ボタンでリセットのみ

**three.js/pixi.jsは両可視化とも未導入。** design-directions.mdの「ショーケースティアはWebGL適性◎」という記述はあるが、まずは追加依存なしのCanvas 2D + CSSで様子を見る方針(ユーザーとの合意事項)。パーティクル演出等が必要になったタイミングで再検討する
...
- **可視化はソート4種(バブル/選択/挿入/クイック)+経路探索2種(BFS/DFS)の計6件のみ**。残り157件は詳細ページを開いても「準備中」の破線パネルが表示されるだけ
- **Web Worker上でのアルゴリズム実行、状態スナップショット/diffベースの記録、IndexedDBキャッシュは未実装**。現状の可視化は全て本来のアーキテクチャ設計(Web Worker + diff記録)を通さない、クライアント側で全ステップ事前生成する暫定実装
...
1. Web Worker + 状態スナップショット(diffベース)の設計・実装。現状の可視化(クライアント側で全ステップ事前生成)を本来のアーキテクチャに置き換える
2. マージソート・ヒープソートなど残りのソート、DP(ナップサック等)・A*探索など、可視化対応アルゴリズムをさらに拡張する
...
6. three.js/pixi.jsによるショーケース級演出の検討(現状はCanvas 2Dのみ)
