...
- **可視化は181件(2026-07-14)**。ソート23種+配列探索26種(新設eulers-method/runge-kutta-method/trapezoidal-rule/simpsons-rule/monte-carlo-integration含む)+グリッド経路探索9種+グラフ41種(新設negamax/expectimax/iterative-deepening-minimax含む)+DP69種(新設fermat-primality-test/pollards-p-minus-1/tonelli-shanks/montgomery-multiplication/elgamal-encryption/elliptic-curve-cryptography/toom-cook-multiplication/gaussian-elimination/lu-decomposition/least-squares/power-iteration/discrete-convolution/qr-decomposition/multivariate-newton-method/needleman-wunsch/smith-waterman/nussinov-algorithm/multiple-sequence-alignment/viterbi-algorithm/forward-backward-algorithm/cky-algorithm/conditional-random-field/optimal-binary-search-tree/palindrome-partitioning/burst-balloons-dp含む)+木構造6種+トライ木2種+文字列パターンマッチング5種。残り186件(デザインパターン23件を除く実質候補は約163件)は詳細ページを開いても「準備中」の破線パネルが表示されるだけ(コンテンツの充実化と可視化対応は別軸)。ゲーム木データセット(GAME_TREE_NODES/EDGES、末端値`[2,3,8,1,7,4,6,9]`)はminimax/alpha-beta-pruning/monte-carlo-tree-search/negamax/expectimax/iterative-deepening-minimaxの6アルゴリズムで共有(negamaxは同じ根の値7に一致することを確認、iterative-deepening-minimaxは深さ制限3で同じ真値7に収束)、ODEの解e^xはeulers-method/runge-kutta-methodの2アルゴリズムで、積分f(x)=x²(真値64/3)はtrapezoidal-rule/simpsons-rule/monte-carlo-integrationの3アルゴリズムで、HMMパラメータ(天気/行動の教科書例)はviterbi-algorithm/forward-backward-algorithmの2アルゴリズムで共有——「同じ入力を複数アルゴリズムで比較できるようにする」という可視化の設計方針を継続している
