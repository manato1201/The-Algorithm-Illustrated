...
最終更新: 2026-07-09(ソート6種+経路探索2種+DP可視化、Web Worker化を追加)
...
1. 状態スナップショットのdiffベース記録・IndexedDBキャッシュの設計・実装(Web Worker化自体は完了、Event Sourcing的な差分記録が残作業)
2. A*探索・DFS/BFS以外のグラフアルゴリズム、他のDP(LCS等)など、可視化対応アルゴリズムをさらに拡張する
