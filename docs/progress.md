...
最終更新: 2026-07-13(ユーザーの「両方並行して進めてください」(深堀りと可視化)の指示を受け、まず可視化(課題b)を9件拡張(121件→130件)。GraphVisualizerで新設した**ゲーム木データセット**(深さ3の完全二分木、末端値`[2,3,8,1,7,4,6,9]`をminimax/alpha-beta-pruning/monte-carlo-tree-searchの3アルゴリズムで共有)を使い、minimax(深さ優先で末端から評価値を伝播)・alpha-beta-pruning(同じ木でα/βを追跡し枝刈り)・monte-carlo-tree-search(UCB1による選択→展開→ランダムロールアウト→バックプロパゲーションの24回反復、決定論的LCG使用)を実装。同じくGraphVisualizerで**トンプソン構成法**(正規表現`a(b|c)*`を記号・連接・選択・繰り返しの4部品から実際に組み立てるNFA構築、コンビネータ関数として実装)・**部分集合構成法**(NFA→DFA、ε閉包を使った本物のsubset construction)・**DFA最小化**(Mooreの分割改良法、Hopcroftのアルゴリズムと同じ最終結果を与える等価類分割)を実装、この3つは同じNFA/DFAデータを順番に処理する一連のパイプラインとして繋がっている。SearchVisualizerで**newton-method・bisection-method・secant-method**を、3アルゴリズム共通の対象関数`f(x)=x³-x-2`(実根≈1.521380)に対する反復収束として実装(hill-climbingと同じ「地形」表示パターンを再利用)。node --experimental-strip-typesで(1)minimaxの結果が独立実装したbrute-force評価と完全一致、(2)alpha-beta-pruningが同じ結果に到達しつつ実際に1頂点を枝刈りすること(手動トレースと一致)、(3)MCTSが24回反復中22回で真に最適な分岐に収束すること、(4)構築したNFAが10状態→DFA4状態→最小化後2状態という意味のある削減を示すこと、(5)数値計算3種全てが真の根に収束することを確認。tsc/eslint/build(270ルート)/dev+curl(9新規ページ+回帰2ページ)も成功。
...
- **content/algorithms/は263件(24カテゴリ)**。既存163件(15カテゴリ)は`## 概要`・`## 仕組み`・`## 特性・トレードオフ`の3見出し構成でコンテンツ充実化済み(2026-07-12完了)。新設した「ゲーム」(12件)・「数値計算」(12件)・「コンピュータビジョン」(12件)・「自然言語処理」(12件)・「コンパイラ・構文解析」(12件)・「バイオインフォマティクス」(10件)・「並行処理・並列アルゴリズム」(10件)・「スケジューリング」(10件)・「制御・ロボティクス」(10件)の9カテゴリ(計100件、2026-07-13)も同じ3見出し構成で新規作成済み。デザインパターンの`## 特性・トレードオフ`は他カテゴリのBig-O計算量ではなく、パターン固有のトレードオフ(拡張性とクラス数増加の綱引き、カプセル化との緊張関係など)を軸に記述している
...
- **課題aの「新カテゴリ追加」フェーズは一区切り**。ロードマップに挙げていた8候補(コンピュータビジョン/自然言語処理/コンパイラ・構文解析/バイオインフォマティクス/数値計算・信号処理/並行処理・並列アルゴリズム/スケジューリング/制御・ロボティクス)は全て新設完了。ユーザーの次の指示は「深堀り、視覚化表現の実装とすすんでいきましょう」——次フェーズは(1)既存24カテゴリのサブカテゴリ内でのバリエーション追加(深堀り)、(2)可視化(課題b)の継続、の2軸
...
**ユーザーとの合意事項(2026-07-13、課題aの着手方針)**: 課題aに着手するにあたり3点を確認: (1)Gameアルゴリズムをゲームアイ・意思決定系(Minimax/Alpha-Beta/MCTS等)+手続き型コンテンツ生成(迷路生成/パーリンノイズ等)+数理ゲーム理論(ナッシュ均衡等)の3本柱全てを対象に含める、(2)新カテゴリの追加(裾野を広げる)と既存カテゴリの深堀り(バリエーションを増やす)を並行して進める、(3)先にカテゴリー分類・検索UIを再設計してからコンテンツを大量生成する。この方針に沿って第1弾(subcategoryフィールド追加+カタログ絞り込みUI+Gameカテゴリ12件パイロット)を実施し、163件→175件に到達した。続けて「新カテゴリから着手してください」(第2弾: 数値計算12件)→「新カテゴリを続けてください。全ての新カテゴリを追加し終わったら深堀り、視覚化表現の実装とすすんでいきましょう」(第3弾: 残り7カテゴリ76件を一括新設)という指示で、**新カテゴリ追加フェーズが完了**(187件→263件、24カテゴリ)。**次フェーズはユーザー自身が明言した通り「深堀り」と「可視化(課題b)」の2本立て**——新カテゴリの追加はいったん区切りとし、以降は既存24カテゴリの深堀りと可視化実装を進める。
...
2. **アルゴリズム数を10倍(約1630件)に拡大する**(課題a、着手済み・263件/1630件)。カテゴリ・サブカテゴリ基盤(`CATEGORY_TAXONOMY`)は確立済み、**新カテゴリ追加フェーズは2026-07-13時点で一区切り**(全24カテゴリ、ロードマップに挙げていた候補は出し尽くした)。以降は以下を並行して進める:
   - **既存24カテゴリそれぞれのサブカテゴリ内でのバリエーション追加(深堀り)が次の主軸**。例えばグラフの「最短路」ならSPFA・多始点ダイクストラ等、ゲームの「ゲームAI・意思決定」ならUCT・反復深化ミニマックス等、数値計算の「求根アルゴリズム」なら多変数ニュートン法等、コンパイラ・構文解析の「字句解析」ならAho-CorasickをベースにしたAho-Corasick+DFA融合等、バイオインフォマティクスの「構造予測」ならZukerアルゴリズム(Nussinov法の自由エネルギー拡張版)等、教科書に載っている定番アルゴリズムのうちまだ収録していないものをカテゴリごとに掘り下げる。深堀りに完全に上限はないため、目安として各カテゴリ+5〜10件程度を1バッチの単位とする
   - さらに厳密に「新カテゴリ」を追加する余地(候補: ゲーム理論の協力ゲーム(シャープレイ値等)、暗号理論の発展(楕円曲線暗号等)、量子計算の基礎アルゴリズム等)も将来的にはあるが、**ユーザーの指示に従い、いったん深堀り・可視化を優先する**
   - 各バッチは既存の運用(小規模バッチ+`node --experimental-strip-types`等での独立検証(frontmatter整合性・内部リンク検証を含む)+tsc/eslint/build/dev確認+ドキュメント更新)を踏襲する
3. **可視化(課題b)を継続**。現在130件/263件(デザインパターン23件を除く実質可視化候補は約133件)。**ゲーム(minimax/alpha-beta-pruning/monte-carlo-tree-search)・コンパイラ構文解析の一部(thompson-construction/subset-construction/dfa-minimization)・数値計算の一部(newton-method/bisection-method/secant-method)の9件は2026-07-13完了**。次の有力候補: 新設9カテゴリの残り(バイオインフォマティクスのneedleman-wunsch/smith-waterman(DPTableVisualizerで編集距離と同じ区間DP構造)、並行処理のdining-philosophers(GraphVisualizerで状態遷移)等)、データ構造・文字列の残りの数件、機械学習の多次元手法4件、計算幾何7件+シミュレーション群知能2件(新規2Dコンポーネントが必要)
