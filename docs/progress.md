...
- **可視化はソート23種(新設gnome-sort/odd-even-sort/patience-sorting/tournament-sort/pigeonhole-sort/flashsort含む)+配列探索21種(新設galloping-search含む)+グリッド経路探索9種(新設best-first-search/bidirectional-search含む)+グラフ38種(新設minimax/alpha-beta-pruning/monte-carlo-tree-search/thompson-construction/subset-construction/dfa-minimization/spfa/reverse-delete-algorithm/articulation-points/bridges-finding含む)+DP44種(新設word-break-problem/unbounded-knapsack/partition-problem/assembly-line-scheduling/fractional-knapsack含む)+木構造6種+トライ木2種+文字列パターンマッチング5種の計148件**(2026-07-13)。残り169件(デザインパターン23件を除く実質可視化候補は約169件)は詳細ページを開いても「準備中」の破線パネルが表示されるだけ(コンテンツの充実化と可視化対応は別軸)。ゲーム木データセット(GAME_TREE_NODES/EDGES、末端値`[2,3,8,1,7,4,6,9]`)はminimax/alpha-beta-pruning/monte-carlo-tree-searchの3アルゴリズムで共有、NFA/DFAデータセット(正規表現`a(b|c)*`)はthompson-construction→subset-construction→dfa-minimizationの3アルゴリズムでパイプライン的に共有、求根関数`f(x)=x³-x-2`はnewton-method/bisection-method/secant-methodの3アルゴリズムで共有、新設のBRIDGE_NODES/EDGES(三角形2つを辺C-Dで繋いだグラフ)はarticulation-points/bridges-findingの2アルゴリズムで共有——「同じ入力を複数アルゴリズムで比較できるようにする」という可視化の設計方針を、深堀りバッチ1の可視化でも踏襲した
...
- **可視化の正しさの検証は`npm run verify`(`scripts/verify-visualizations.mjs`)で常時再実行できる(2026-07-13追加)**。全148件の可視化に対し348件のアサーションを実施し独立実装のbrute-force・既知の正解値と突き合わせる(ソート/探索/パス探索は完全網羅、グラフ/DP/木構造は代表的な既知値ベース)。これまでの「バッチごとに使い捨てスクリプトで検証して削除」という運用をやめ、この1ファイルに検証ロジックを積み上げていく運用に変更した——新しいバッチを追加するたびにここへ検証ブロックを追記すること
...
3. **可視化(課題b)を継続**。現在148件/317件(デザインパターン23件を除く実質可視化候補は約169件)。**ゲーム(minimax/alpha-beta-pruning/monte-carlo-tree-search)・コンパイラ構文解析の一部(thompson-construction/subset-construction/dfa-minimization)・数値計算の一部(newton-method/bisection-method/secant-method)の9件、深堀りバッチ1のソート6種+パス探索2種+配列探索1種の9件、深堀りバッチ1のグラフ4種(spfa/reverse-delete-algorithm/articulation-points/bridges-finding)+DP5種(word-break-problem/unbounded-knapsack/partition-problem/assembly-line-scheduling/fractional-knapsack)の9件は2026-07-13完了**。次の有力候補: 深堀りバッチ1の残り36件のうち、GraphVisualizerで表現しやすいグラフ5種(hungarian-algorithm/stable-marriage-problem/push-relabel-max-flow=既存Dinic・Edmonds-Karpと同系統/hierholzer-algorithm/two-sat=Tarjanと同系統のSCC応用)、DPTableVisualizerで表現しやすいDP2種(palindrome-partitioning/optimal-binary-search-tree/burst-balloons-dpは区間DPで既存matrix-chain-multiplicationと同系統)・貪欲法1種(job-sequencing-with-deadlines)、新設9カテゴリの残り(バイオインフォマティクスのneedleman-wunsch/smith-waterman等)、データ構造・文字列の残りの数件、機械学習の多次元手法4件、計算幾何7件+シミュレーション群知能2件(新規2Dコンポーネントが必要)
