...
- **より高精度な代替手法(アンセンテッドカルマンフィルタ)**: ヤコビ行列による1次近似では精度が不十分な場合、シグマ点と呼ばれる代表的な点をいくつか選んで非線形変換を直接適用する「[アンセンテッドカルマンフィルタ(UKF)](/algorithms/unscented-kalman-filter)」が、ヤコビ行列の計算(解析的な微分の導出)を避けつつEKFより高い精度を実現する発展形として使われることがある
