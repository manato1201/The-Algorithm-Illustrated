...
現時点ではNext.jsのスキャフォールド・デザインシステム・カタログ画面・アルゴリズム詳細ページ(ソート6種+経路探索2種+DP1種が可視化対応、Web Worker経由)・Markdownベースの実データモデル(163件)が実装済み。詳細は [docs/progress.md](docs/progress.md) を参照。
...
src/
  ...
  components/
    ...
    visualizer/          可視化コンポーネント一式。ソート6種+経路探索2種+DP1種+再生コントロール共通部品(useStepPlayer/PlaybackControls)+Web Worker連携フック(useWorkerFrames)
  lib/
    ...
    pathfinding-visualizers.ts  BFS/DFSのステップ列生成ロジック(固定迷路)
...
- ✅ アルゴリズム詳細ページ(163件を静的生成。可視化はソート6種(バブル/選択/挿入/マージ/ヒープ/クイック)+経路探索2種(BFS/DFS)+DP1種(0-1ナップサック)、他154件は準備中表示)
- ✅ 可視化のステップ列生成をWeb Worker化(algorithm-worker.ts)
- ✅ Markdownベースの実データモデル(content/algorithms/)。ソートカテゴリ17件は概要・仕組み・特性トレードオフを充実化済み、残り14カテゴリ137件は簡易生成のまま
- ⬜ 比較画面・更新情報(RSS)画面
- ⬜ 状態スナップショットのdiffベース記録・IndexedDBキャッシュ(Event Sourcing的な差分記録は未実装)
- ⬜ Vercel Edge Functions(BFF)
- ⬜ 残り14カテゴリ137件のコンテンツ充実化
