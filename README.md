...
現時点ではNext.jsのスキャフォールド・デザインシステム・カタログ画面・アルゴリズム詳細ページ(ソート17種+配列探索20種(...カダンのアルゴリズム/エラトステネスの篩/フェニック木/ブルームフィルタ/山登り法/焼きなまし法/タブーサーチ/勾配降下法/k近傍法/マナカーのアルゴリズム/ニュートン法/二分法/割線法)+グリッド経路探索7種(BFS/DFS/ダイクストラ法/A*探索/IDDFS/ライフゲーム/ラングトンのアリ)+グラフ34種(...ブリー・アルゴリズム/二相コミット/Raft/Paxos/ベクタークロック/決定木/高速フーリエ変換/分枝限定法/minimax/アルファベータ枝刈り/モンテカルロ木探索/トンプソン構成法/部分集合構成法/DFA最小化)+DP39種(...LRUキャッシュ/TF-IDF/BM25/RRF/パーセプトロン/誤差逆伝播法/ナイーブベイズ/遺伝的アルゴリズム/モンテカルロ法/巡回セールスマン問題(bitDP)/MinHash・LSH/接尾辞配列/シンプレックス法)+木構造6種(BST/AVL木/Treap/赤黒木/スプレー木/区間木)+トライ木2種(トライ木/Aho-Corasick法)+文字列パターンマッチング5種(KMP法/ラビン-カープ法/Z algorithm/ボイヤー・ムーア法/連長圧縮)の計130件が可視化対応、Web Worker経由)・比較画面(可視化グリッド付き)・更新情報(RSS)画面・Aboutページ・Markdownベースの実データモデル(263件、全24カテゴリ。category→subcategoryの2階層分類+カタログの絞り込みチップUIに対応、新設9カテゴリ計100件(ゲーム/数値計算/コンピュータビジョン/自然言語処理/コンパイラ・構文解析/バイオインフォマティクス/並行処理・並列アルゴリズム/スケジューリング/制御・ロボティクス)を含む)が実装済み。アルゴリズム総数を10倍規模(約1600件超)に拡大する取り組みに着手済み(新カテゴリ追加フェーズは一区切り、次は深堀り・可視化の2軸)。詳細は [docs/progress.md](docs/progress.md) を参照。
...
- ✅ アルゴリズム詳細ページ(263件を静的生成。可視化はソート17種+配列探索20種(線形/二分/三分/ジャンプ/補間/指数/フィボナッチ探索/カダンのアルゴリズム/エラトステネスの篩/フェニック木/ブルームフィルタ/山登り法/焼きなまし法/タブーサーチ/勾配降下法/k近傍法/マナカーのアルゴリズム/ニュートン法/二分法/割線法)+グリッド経路探索7種(BFS/DFS/ダイクストラ法/A*探索/IDDFS/ライフゲーム/ラングトンのアリ)+グラフ34種(ベルマン・フォード法/プリム法/クラスカル法/トポロジカルソート/ボルーフカ法/Union-Find/Tarjanの強連結成分分解/Edmonds-Karp法/Dinic法/Ford-Fulkerson法/ホップクロフト・カープ法/カーンのアルゴリズム/ジョンソンのアルゴリズム/ハフマン符号化/フロイドの循環検出法/セグメント木/スキップリスト/PageRank/HITS/一貫性ハッシュ法/ブリー・アルゴリズム/二相コミット/Raft/Paxos/ベクタークロック/決定木/高速フーリエ変換/分枝限定法/minimax/アルファベータ枝刈り/モンテカルロ木探索/トンプソン構成法/部分集合構成法/DFA最小化)+DP39種+木構造6種(二分探索木/AVL木/Treap/赤黒木/スプレー木/区間木)+トライ木2種(トライ木/Aho-Corasick法)+文字列パターンマッチング5種(KMP法/ラビン-カープ法/Z algorithm/ボイヤー・ムーア法/連長圧縮)の計130件、他133件は準備中表示)
...
- ⬜ 残りアルゴリズム(133件)の可視化拡張(データ構造カテゴリは16件中13件対応・残りb-tree/kd-tree/quad-treeの3件のみ、文字列カテゴリは11件中9件対応・残りburrows-wheeler-transform/suffix-automatonの2件のみ、分散システム・最適化・DP・数論暗号は全件完了、機械学習は10件中6件対応・残りk-means/pca/svm/random-forest、シミュレーション/群知能は5件中3件対応・残りant-colony-optimization/boids/particle-swarm-optimization、計算幾何(7件)は未着手・新規2Dコンポーネントが必要、新設9カテゴリ(計100件)はminimax/alpha-beta-pruning/monte-carlo-tree-search・thompson-construction/subset-construction/dfa-minimization・newton-method/bisection-method/secant-methodの9件が対応済み・残り91件、デザインパターン23件は対象外と判断(未確認))
