...
| `npm run verify`   | 可視化(181件)の正しさを独立実装のbrute-force・既知の正解値と突き合わせて検証(`scripts/verify-visualizations.mjs`) |
...
- ✅ アルゴリズム詳細ページ(367件を静的生成。可視化はソート23種+配列探索26種+グリッド経路探索9種+グラフ41種(negamax/expectimax/iterative-deepening-minimax含む)+DP69種(数論・暗号7種/数値計算7種/配列アラインメント4種/系列タグ付け4種/区間DP3種を新設)+木構造6種(二分探索木/AVL木/Treap/赤黒木/スプレー木/区間木)+トライ木2種(トライ木/Aho-Corasick法)+文字列パターンマッチング5種(KMP法/ラビン-カープ法/Z algorithm/ボイヤー・ムーア法/連長圧縮)の計181件、他186件は準備中表示)
...
- ⬜ 残りアルゴリズム(186件)の可視化拡張(2026-07-14に33件追加(数論・暗号7件/数値計算・線形代数12件/配列アラインメントDP4件/系列タグ付けDP4件/区間・ナップサックDP3件/ゲーム木3件)、148件→181件。次の候補: グラフ系9件(hungarian-algorithm/stable-marriage-problem/push-relabel-max-flow/hierholzer-algorithm/two-sat/textrank/register-allocation-graph-coloring/de-bruijn-graph-assembly/suffix-automaton)、分散・並行処理17件(dining-philosophers等10件+lock-free-stack-cas等7件)、スケジューリング14件、データ構造・文字列圧縮13件、系統樹3件。デザインパターン23件・計算幾何7件・機械学習の多次元手法4件等は新規2Dコンポーネントが必要なため引き続き優先度低)
