...
現時点ではNext.jsのスキャフォールド・デザインシステム・カタログ画面・アルゴリズム詳細ページ(ソート4種+経路探索2種が可視化対応)が実装済み。詳細は [docs/progress.md](docs/progress.md) を参照。
...
src/
  ...
  components/
    ...
    visualizer/          可視化(Canvas 2D)。ソート4種+経路探索2種+再生コントロール共通部品
  lib/
...
- ✅ アルゴリズム詳細ページ(163件を静的生成。可視化はソート4種(バブル/選択/挿入/クイック)+経路探索2種(BFS/DFS)、他157件は準備中表示)
- ⬜ 比較画面・更新情報(RSS)画面
- ⬜ Web Worker上での状態スナップショット/diff記録・IndexedDBキャッシュ(現状の可視化はクライアント側の簡易実装で代用)
