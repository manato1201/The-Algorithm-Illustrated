...
現時点ではNext.jsのスキャフォールド・デザインシステム・カタログ画面・アルゴリズム詳細ページ(ソート17種+配列探索7種+グリッド経路探索4種(BFS/DFS/ダイクストラ法/A*探索)+グラフ5種+DP10種+木構造3種(BST/AVL木/Treap)+文字列パターンマッチング3種(KMP法/ラビン-カープ法/Z algorithm)の計49件が可視化対応、Web Worker経由)・比較画面・更新情報(RSS)画面・Aboutページ・Markdownベースの実データモデル(163件、全15カテゴリのコンテンツ充実化完了)が実装済み。詳細は [docs/progress.md](docs/progress.md) を参照。
...
src/
  app/
    ...
    compare/            比較画面(最大4件のアルゴリズムを並べて比較)
  ...
  components/
    ...
    visualizer/          可視化コンポーネント一式。ソート17種+配列探索7種+グリッド経路探索4種+グラフ5種+DP10種+木構造3種+文字列パターンマッチング3種+再生コントロール共通部品(useStepPlayer/PlaybackControls)+Web Worker連携フック(useWorkerFrames)+パーティクル演出(ParticleBurstLayer、pixi.js)
  lib/
    ...
    graph-visualizers.ts    グラフ5種(ベルマン・フォード法/プリム法/クラスカル法/トポロジカルソート/ボルーフカ法)のステップ列生成ロジック(ノードリンク図)
    dp-visualizers.ts      DP10種(0-1ナップサック/LCS/編集距離/硬貨両替/棒の切り出し/部分和/LIS/最長回文部分列/フロイド-ワーシャル法/Sparse Table)のステップ列生成ロジック
    tree-visualizers.ts    木構造3種(二分探索木/AVL木/Treap)のステップ列生成ロジック(挿入シーケンス+回転)
    string-visualizers.ts  文字列パターンマッチング3種(KMP法/ラビン-カープ法/Z algorithm)のステップ列生成ロジック
...
- ✅ アルゴリズム詳細ページ(163件を静的生成。可視化はソート17種+配列探索7種+グリッド経路探索4種(BFS/DFS/ダイクストラ法/A*探索)+グラフ5種(ベルマン・フォード法/プリム法/クラスカル法/トポロジカルソート/ボルーフカ法)+DP10種+木構造3種(二分探索木/AVL木/Treap)+文字列パターンマッチング3種(KMP法/ラビン-カープ法/Z algorithm)の計49件、他114件は準備中表示)
...
- ✅ 比較画面(/compare)・更新情報(RSS)画面(/updates + /api/updates)・Aboutページ(/about)
...
- ⬜ 残りアルゴリズム(114件)の可視化拡張(データ構造カテゴリは16件中3件、文字列カテゴリは11件中3件のみ対応)、比較画面での可視化表示、アルゴリズム数の10倍化(いずれも次フェーズの計画中)、モーション停止ボタンUI、比較画面の特性トレードオフ深掘り
