...
```
content/
  algorithms/           アルゴリズム記事(実データモデル本体)。<id>.md × 187件、frontmatter+Markdown本文
src/
  app/
    layout.tsx        ルートレイアウト。フォント読み込み+AppShellでラップ
    page.tsx           トップページ(カタログ画面を呼び出すだけ)
    globals.css         デザイントークン(:root)とベーススタイル
    algorithms/[id]/    アルゴリズム詳細ページ(動的ルート、187件を静的生成)
...
- ✅ カタログ画面(検索・カテゴリ/サブカテゴリ絞り込みチップ、187件)
- ✅ アルゴリズム詳細ページ(187件を静的生成。可視化はソート17種+配列探索17種(線形/二分/三分/ジャンプ/補間/指数/フィボナッチ探索/カダンのアルゴリズム/エラトステネスの篩/フェニック木/ブルームフィルタ/山登り法/焼きなまし法/タブーサーチ/勾配降下法/k近傍法/マナカーのアルゴリズム)+グリッド経路探索7種(BFS/DFS/ダイクストラ法/A*探索/IDDFS/ライフゲーム/ラングトンのアリ)+グラフ28種(ベルマン・フォード法/プリム法/クラスカル法/トポロジカルソート/ボルーフカ法/Union-Find/Tarjanの強連結成分分解/Edmonds-Karp法/Dinic法/Ford-Fulkerson法/ホップクロフト・カープ法/カーンのアルゴリズム/ジョンソンのアルゴリズム/ハフマン符号化/フロイドの循環検出法/セグメント木/スキップリスト/PageRank/HITS/一貫性ハッシュ法/ブリー・アルゴリズム/二相コミット/Raft/Paxos/ベクタークロック/決定木/高速フーリエ変換/分枝限定法)+DP39種+木構造6種(二分探索木/AVL木/Treap/赤黒木/スプレー木/区間木)+トライ木2種(トライ木/Aho-Corasick法)+文字列パターンマッチング5種(KMP法/ラビン-カープ法/Z algorithm/ボイヤー・ムーア法/連長圧縮)の計121件、他66件は準備中表示)
- ✅ 可視化のステップ列生成をWeb Worker化(algorithm-worker.ts、単一Workerを使い回す設計)
- ✅ Markdownベースの実データモデル(content/algorithms/)。全17カテゴリ・187件全てで概要・仕組み・特性トレードオフを充実化済み(デザインパターン(GoF23種)、新設のゲーム・数値計算カテゴリ計24件を含めて完了)。category→subcategoryの2階層分類(CATEGORY_TAXONOMY)に対応
...
- ⬜ 残りアルゴリズム(66件)の可視化拡張(データ構造カテゴリは16件中13件対応・残りb-tree/kd-tree/quad-treeの3件のみ、文字列カテゴリは11件中9件対応・残りburrows-wheeler-transform/suffix-automatonの2件のみ、分散システム・最適化・DP・数論暗号は全件完了、機械学習は10件中6件対応・残りk-means/pca/svm/random-forest、シミュレーション/群知能は5件中3件対応・残りant-colony-optimization/boids/particle-swarm-optimization、計算幾何(7件)は未着手・新規2Dコンポーネントが必要、ゲームカテゴリ12件・数値計算カテゴリ12件は未着手(minimax/alpha-beta-pruning/monte-carlo-tree-searchはbranch-and-boundと同じゲーム木事前構築パターン、newton-method/bisection-method等はhill-climbing等と同じSearchVisualizerの地形表示パターンで対応できる見込み)、デザインパターン23件は対象外と判断(未確認))
- ⬜ アルゴリズム数を10倍規模(約1600件超)に拡大(課題a、着手済み・187件。category→subcategory分類基盤+新設ゲーム・数値計算カテゴリ計24件。新カテゴリ追加候補と進め方は[docs/progress.md](docs/progress.md)のロードマップ参照)
